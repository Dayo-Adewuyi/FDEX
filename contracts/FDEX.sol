...
contract Exchange is ERC20 {
    ...
    // Exchange is inheriting ERC20, becase our exchange would keep track of LP tokens
    ...
    /**
     * @dev Returns the amount of  Tokens held by the contract
     */
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        */
        if (TokenReserve == 0) {
            // Transfer the  address from the user's account to the contract
            ...
            /* 
                ...
                `Ether` and determine according to the ratio how many tokens
            ...
            */
            ...
            // Ration here is -> (TokenAmount user can add/TokenReserve in the contract) = (Eth Sent by the user/Eth Reserve in the contract);
            // So doing some maths, (TokenAmount user can add) = (Eth Sent by the user * TokenReserve /Eth Reserve);
            uint TokenAmount = (msg.value * TokenReserve) / (ethReserve);
            require(
                _amount >= TokenAmount,
                "Amount of tokens sent is less than the minimum tokens required"
            );
            // transfer only (TokenAmount user can add) amount of `tokens` from users account
            // to the contract
    ...
    function removeLiquidity(uint _amount) public returns (uint, uint) {
        ...
        // on a ratio
        // = (current token reserve * amount of LP tokens that user wants to withdraw) / (total supply of LP tokens)
        ...
        // Transfer `TokenAmount` of tokens from the user's wallet to the contract
